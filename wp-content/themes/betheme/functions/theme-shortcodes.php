...
if (! function_exists('sc_popup')) {
	function sc_popup($attr, $content = null)
	{
		...
		if ($button) {
			$output .= '<a href="#'. esc_attr($uid) .'" rel="prettyphoto" class="popup-link button"><span class="button_label">'. wp_kses($title, mfn_allowed_html()) .'</span></a>';
		} else {
...
if (! function_exists('sc_image')) {
	function sc_image($attr, $content = null)
	{
		...
		if ( $size || defined( 'ICL_SITEPRESS_VERSION' ) ) {
	...
	function sc_quick_fact($attr, $content = null)
	{
		...
		$output = '<div class="quick_fact align_'. esc_attr($align) .' '. esc_attr($animate_math) .'">';
			...
			if ($animate) {
				...
				if ($heading) {
					$output .= '<'. esc_attr( $heading_tag ) .' class="title">'. wp_kses($heading, mfn_allowed_html()) .'</'. esc_attr( $heading_tag ) .'>';
				}
