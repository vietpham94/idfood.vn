(function ($) {
    ...
    var PremiumUnfoldHandler = function ($scope, $) {
        ...
        //Get fold height setting value
        var foldHeight = settings.foldHeight;
        ...
        //We need to fold content on page load
        $unfoldElem.find(".premium-unfold-content").css({
            height: foldHeight
        });
        ...
        $unfoldElem.on('click', '.premium-button', function (e) {
            ...
            if ($unfoldElem.find(".premium-unfold-content").hasClass("toggled")) {
                ...
                $unfoldElem.find(".premium-unfold-button-text").text(settings.unfoldText);

                $unfoldElem.find(".premium-unfold-content").animate({
                ...
                $unfoldElem.find(".premium-button i").removeClass(settings.buttonUnfoldIcon).addClass(settings.buttonIcon);
                ...
                $unfoldElem
                    ...
                    .removeClass(settings.buttonIcon)
                    .addClass(settings.buttonUnfoldIcon);
            }
        });
    };
        ...
        if (_this.length > 0) {
            if (settings.responsive) {
                if (settings.hideMobs) {
                    if (
                        $(window).outerWidth() < parseInt(settings.mobSize)
                    ) {
            ...
            function notificationSetCookie(cookieName, cookieValue) {
                ...
                expire.setTime(
                    today.getTime() + 3600000 * settings.interval
                );
            ...
            if (
                ...
                settings.position === "premium-notbar-relative"
            ) {
                $($barElem).detach();
    ...
    var PremiumChartHandler = function ($scope, $) {
        var premiumChart = $scope.find(".premium-chart-container"),
            settings = premiumChart.data("settings"),
            currentDevice = elementorFrontend.getCurrentDeviceMode(),
            type = settings.type,
            eventsArray = [
                "mousemove",
    ...
    var PremiumFacebookHandler = function ($scope, $) {
        ...
        var premiumFacebookFeedElement = $scope.find(".premium-facebook-feed-wrapper"),
            ...
            carousel = premiumFacebookFeedElement.data("carousel");
        ...
        function handleFacebookFeed() {
            if (carousel) {
                ...
                var autoPlay = premiumFacebookFeedElement.data("play"),
                    speed = premiumFacebookFeedElement.data("speed"),
    ...
    window.CBPFWTabs = function (t, settings) {
        ...
        self._show = function (tabIndex, tab) {
            ...
            self.items.eq(self.current).addClass("content-current");
        };
